...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PDG);
    ...
    if (!settings.contains("nAnonymizePdgAmount"))
        settings.setValue("nAnonymizePdgAmount", 1000);

    nAnonymizePdgAmount = settings.value("nAnonymizePdgAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePdgAmount"))
        SoftSetArg("-anonymizepdgamount", settings.value("nAnonymizePdgAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pdg.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePdgAmount:
            return QVariant(nAnonymizePdgAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePdgAmount:
            nAnonymizePdgAmount = value.toInt();
            settings.setValue("nAnonymizePdgAmount", nAnonymizePdgAmount);
            emit anonymizePdgAmountChanged(nAnonymizePdgAmount);
            break;
        case CoinControlFeatures:
