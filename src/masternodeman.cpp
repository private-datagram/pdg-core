...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!obfuScationSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
            ...
            Misbehaving(pfrom->GetId(), 33, __FILE__, __LINE__);
            return;
        }
            ...
            if (!isLocal && Params().NetworkID() == CBaseChainParams::MAIN) {
                ...
                if (i != mAskedUsForMasternodeList.end()) {
                    ...
                    if (GetTime() < t) {
                        ...
                        Misbehaving(pfrom->GetId(), 34, __FILE__, __LINE__);
                        return;
                    }
                }
                int64_t askAgain = GetTime() + MASTERNODES_DSEG_SECONDS;
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 37712) return;
        } else if (addr.GetPort() == 37712)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 PDG tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 PDG tx -> 1 confirmation
