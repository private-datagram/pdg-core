...
// The zPDG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPDG is a hash of the serial
...
{
    //Create an output returning the zPDG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPIVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPDG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPDG", __func__);
...
{
    //The unique identifier for a PDG stake is the outpoint
