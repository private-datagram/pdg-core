...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    ...
    obj.push_back(Pair("zPDGsupply", zpivObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"pdgaddress\"\n"
            "\nReturn information about the given pdg address.\n"

            "\nArguments:\n"
            "1. \"pdgaddress\"     (string, required) The pdg address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"pdgaddress\", (string) The pdg address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: PDG address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are pdg addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) pdg address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"pdgaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"pdgaddress\"  (string, required) The pdg address to use for the signature.\n"
