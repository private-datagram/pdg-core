...
{
    std::string str;
    str += strprintf("CMutableTransaction(ver=%d, type=%d, vin.size=%u, vout.size=%u, nLockTime=%u)\n",
        nVersion,
        type,
        vin.size(),
        vout.size(),
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // PDG: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
