...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 PDG finalization collateral
            //       will also cover the old 50 PDG finalization collateral.
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mvote") { //Masternode Vote
        ...
        if (!vote.SignatureValid(true)) {
            ...
            if (masternodeSync.IsSynced()) Misbehaving(pfrom->GetId(), 20, __FILE__, __LINE__);
