...
PDG Core version *3.1.0* is now available from:  <https://github.com/private-datagram/pdg-core/releases>
...
PDG Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zPDG Staking is now live! With the release of zPDG staking, there are effectively 2 versions of zPDG, zPDG minted on the 3.0.6 PDG wallet or lower, and zPDG minted on PDG wallet version or higher. New features in this release will require the use of zPDG v2, zPDG minted on this wallet release or later. If you currently hold zPDG v1 and wish to take advantage of zPDG staking and deterministic zPDG, you will need to spend the zPDG v1 to yourself and remint zPDG v2.
In order to take advantage of zPDG staking, you must mint new zPDG v2 and wait for at least 200 confirmations before that zPDG is considered valid for staking.

**Note:**  To find your zPDG version, click the privacy tab, then the zPDG Control button then expand the arrows next to the desired denomination.


### Deterministic zPDG Seed Keys

zPDG is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zPDG outside of the wallet that the zPDG had been minted on. zPDG can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zPDG minting

zPDG minting now only requires 1 further mint (down from 2) to mature. zPDG mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zPDG Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zPDG denomination has been spent or not.



PDG/zPDG Staking and Masternode Rewards
--------------

### PDG, zPDG and Masternode Payment Schedule

To encourage the use of zPDG and increase the PDG zerocoin anonymity set, the PDG payment schedule has been changed to the following:

If a user staking zPDG wins the reward for their block, the following zPDG reward will be:
- 3 zPDG (3 x 1 denominations) rewarded to the staker, 2 PDG rewarded to the masternode owner and 1 PDG available for the budget. This is a total block reward of 6 PDG, up from 5.

If a user staking PDG wins the reward, the following amounts will be rewarded:
- 2 PDG to the PDG staker, 3 PDG to the Masternode owner and 1 PDG available for the budget. This is a total block reward of 6 PDG, up from 5.


### Return change to sender when minting zPDG

Previously, zPDG minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the the contributing address which contained the most PDG and return the change from a zPDG mint to that address.
...
The visual layout of the PDG Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zPDG minting feature. This is enabled by default and the enablezeromint=0 setting in the pdg.conf file will overwrite the GUI option.
-	The percentage of autominted zPDG can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular PDG.
...
### Summary
 ...
 The PDG wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the pdg.conf file, found in the PDG data directory.
- backuppath = <directory / full path>
- zpdgbackuppath = <directory / full path>
...
* If zPDG backup, auto generated name is `wallet-autozpdgbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
The PDG client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running pdgd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the PDG support group, which often times sees users say "I didn't spend that zPDG". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the PDG masternodes, the monthly budget available to be utilised has been increased to 42,000 PDG / month. This PDG only has the opportunity to be raised once per month (paid to winning proposals) with any unused PDG not created by the blockchain.

### Budget Finalization Fee

The PDG finalization fee for successful proposals has now been reduced, this fee is now 5 PDG down from 50 PDG. The total fee outlay for a successful proposal is now a total of 55 PDG.
...
-	Enforced v1 zPDG spends to require a security level of 100
-	Updates to zPDG spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the PDG-Qt masternodes tab
-	Updated PDG-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and PDG's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zPDG Control dialog in the QT wallet, a user is able to see zPDG marked as version 1 or 2.

zPoS (zPDG staking)
--------------

Once a zPDG has over 200 confirmations it becomes available to stake. Staking zPDG will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zPDG. So for example if a 1,000 zPDG denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zPIVs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within PDG's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zPDG, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zPDG that can be recalculated at any time using the seed. Deterministic zPDG allows for users to backup all of their future zPDG by simply recording their seed and keeping it in a safe place (similar to backing up a private key for PDG). The zPDG seed needs to remain in the wallet in order to spend the zPDG after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zPDG data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

PDG's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which PDG used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to PDG using the bug bounty program. The modulus is now correctly imported and PDG's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zPDG v2: zPoS dzPIV ezPIV external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zPDG balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zPDG status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zPDG Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault pdg-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zPDG spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zPDG. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zPDG. (presstab)
